...
export default function SummaryPanel({ selected }) {
  const [actions, setActions] = useState([
    ...
    { id: 2, text: "Validate goods receipt for current cycle", checked: true },
    ...
    <div className="bg-white rounded-xl border border-slate-200 shadow-sm p-4 space-y-5 flex flex-col justify-between">
      <div className="flex items-center justify-between border-b border-slate-100 pb-3">
        <div>
          ...
          <p className="text-[11px] text-blue-600 font-bold mt-0.5 tracking-wide">
      ...
      <div className="grid grid-cols-2 gap-y-3.5 gap-x-3 text-[11px] bg-slate-50/50 p-3 rounded-lg border border-slate-100">
        <div>
          <span className="text-slate-400 block font-medium uppercase text-[9px] tracking-wider mb-0.5">
            Type
          </span>
          <span className="font-semibold text-slate-700 block truncate" title={selected.exceptionType}>
        ...
        <div>
          ...
          <span className="font-extrabold text-slate-900 text-xs">{selected.amount}</span>
        </div>
        <div>
          <span className="text-slate-400 block font-medium uppercase text-[9px] tracking-wider mb-0.5">
            Vendor
          </span>
          <span className="font-semibold text-slate-700 block truncate" title={selected.vendor}>
            {selected.vendor}
          </span>
        </div>
        <div>
      ...
      <div className="space-y-3">
        ...
        <p className="text-[10px] text-slate-400 -mt-1 italic">
          AI suggests the following workflow checklist:
        </p>
      ...
      <div className="pt-2">
        <button
          onClick={() => alert(`Initiating exception workflow strategy for item: ${selected.exceptionId}`)}
